...
namespace CarrinhoCompras.Tests {
    ...
    public class ClienteTests {
        ...
        public void ClienteCarrinho()
        {
            ...
            Profissao = "Desenvolvedor web", DataCadastro = now() },
            new Cliente { ClienteId = 2, Nome = "Marilia Kessia",
            Email = "mkessia@gmail.com", Telefone = "8197499820",
            Endereco = "Rua dos testes Devmedia",
            Profissao = "Professora de lingua inglesa", DataCadastro = now() },
            new Cliente { ClienteId = 3, Nome = "Maitê Dionisio",
            Email = "maite.dionisio@gmail.com", Telefone = "8197499820",
            Endereco = "Rua dos testes Devmedia", Profissao = "Analista de softwares", DataCadastro = now() },
            ...
            Endereco = "Rua dos testes Devmedia", Profissao = "Designer de aplicações", DataCadastro = now() }
            };
            ...
            mockClienteRepositorio.Setup(clienteRepo => clienteRepo.BuscaPorId(It.IsAny<int>())).Returns((int i)
            => clientes.Where(c => c.ClienteId == i).Single());
            // Retornando os clientes por nome
            mockClienteRepositorio.Setup(cliente => cliente.FindByNome(It.IsAny<string>())).Returns((string s)
            => clientes.Where(c => c.Nome == s).Single());
